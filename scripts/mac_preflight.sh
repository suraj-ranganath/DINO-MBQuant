...
mods = ["torch", "torchvision", "gym", "einops", "submitit", "wandb", "hydra", "omegaconf", "pandas", "matplotlib", "yaml", "scipy", "psutil"]
missing = []
for m in mods:
    try:
...
if [[ "$CKPT_BASE_PATH" == "/ABS/PATH/TO/checkpoints_root" ]]; then
...
if [[ -n "$FREE_GB" ]] && [[ "$FREE_GB" -lt 15 ]]; then
  echo "[warn] Low free disk (<15GB). Runs may fail while writing outputs/videos."
fi
...
print(f"[info] mps available={hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()}")
PY
